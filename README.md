...
7. You have the choice of building 3 versions:
    ...
    - For the Retail ROM, run `npm run build:retail` -- this includes the retail checksum validation ccheck

    - For the Rev A ROM, run `npm run build:reva` -- this includes the retail checksum validation ccheck
...
I'll list each conditional block in the order they appear in the code, with:
- **Location**: Where in the code it occurs.
- **Rev=0 (Retail) Code**: What compiles in the original version.
- **Rev=1 (Revision A) Code**: What compiles in the patched version.
- **Description/Comment**: What the difference achieves, likely bug fix rationale, and impact.
...
## .JIM
Documentation for the NHL92 .JIM file format lives here: https://github.com/abdulahmad/EA-NHL-Tools/tree/main/JIM-Tools

## ANIM format
Documentation for the NHL92 .ANIM file format lives here: https://github.com/abdulahmad/EA-NHL-Tools/tree/main/ANIM-To-BMP

## .PAL
.pal files use standard Genesis CRAM format for palette

Below is my assistance in finishing out the documentation for your NHL92 Genesis project. I'll build directly on the README-style content you provided at the end of your query, incorporating refinements, expansions, and clarifications based on the disassembled code (both 68000 sound.asm and Z80 via IDA Pro). I'll integrate insights from the code structure, file extracts, and overall architecture to make it more complete and user-friendly. This includes:

- **Expanded Sections**: Adding details on sound formats (e.g., FM instruments, PCM, command streams), Z80 driver behavior, and how the 68000 and Z80 interact.
- **Corrections/Clarifications**: Fixing minor inaccuracies (e.g., some address labels, tool usage notes) and adding cross-references.
- **New Sections**: Adding "Sound System Overview," "Known Issues/Limitations," and "Modding Tips" to round out the docs.
- **Anything Else You Should Know**: At the end, I'll cover additional insights, potential pitfalls, and resources based on analyzing the code and your extracts.

I've kept the tone technical and focused on the project. If you copy-paste this into your README.md, it should fit seamlessly.
...
### instrument patches
FM patches (32 bytes each) starting Z80 RAM $2A5 (ROM $1190B+). Loaded via $80 command. Params: 4 operators (mul/dt/tl/rs/ar/dr/sr/rr/ssg-eg), alg/fb, stereo. 25+ defined; some hardcoded in handlers (e.g., `fm_instrument_patch_sfx_id_3` at $12DE2 overlaps PCM).
...
- **PCM Overlaps**: Some FM patches overlap PCM (e.g., $10820 in fm_instrument_patch_sfx_31). Modding may corrupt audio.
...
- **Tools**: Use your extract script for assets. For YM2612 editing, try DefleMask or VGM players. Debug Z80 with Gens/GS (breakpoint on $A00000+).
- **Testing**: Build `dev` for no checksum lock. Monitor shared RAM ($FFFFFE40+) for channel states.

## Anything Else You Should Know
- **Code Insights**: The 68000 code has redundancies (e.g., `PUSHO OPT OZ-` for zero-displacement opts, likely assembler quirks). Z80 has inefficient busy-waits (pop hl x3 for ~12 cycles delay per YM write)—common in early Genesis drivers to avoid chip bugs. SFX sharing (e.g., puckwall1_pcm partial reuse) saves space but limits variety.